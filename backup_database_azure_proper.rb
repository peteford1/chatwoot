...
puts "💾 Starting Azure PostgreSQL Flexible Server Backup..."
...
list_cmd = "az postgres flexible-server backup list --resource-group #{azure_config[:resource_group]} --server-name #{azure_config[:server_name]} --output json 2>&1 | tee -a #{log_file}"
existing_backups = `#{list_cmd}`

if $?.success?
  begin
    backups = JSON.parse(existing_backups.split("\n").last)
    puts "   ✅ Found #{backups.length} existing backups"
    backups.each_with_index do |backup, index|
      puts "   #{index + 1}. #{backup['name']} (#{backup['backupType']}) - #{backup['completedTime']}"
    end
  rescue JSON::ParserError
    puts "   ⚠️  Could not parse backup list"
  end
else
...
if backup_success
  ...
  begin
    ...
    json_line = result_lines.find { |line| line.start_with?('{') }
    if json_line
    ...
    puts "   ⚠️  Could not parse backup result details"
...
if backup_success
  ...
  show_cmd = "az postgres flexible-server backup show --resource-group #{azure_config[:resource_group]} --server-name #{azure_config[:server_name]} --backup-name #{backup_name} --output json 2>&1 | tee -a #{log_file}"
  
  backup_details = `#{show_cmd}`
  if $?.success?
    begin
      details = JSON.parse(backup_details.split("\n").last)
      ...
      puts "   ⚠️  Could not parse backup details"
    end
  end
end
...
az postgres flexible-server backup list --resource-group #{azure_config[:resource_group]} --server-name #{azure_config[:server_name]}

# Create new backup
az postgres flexible-server backup create --resource-group #{azure_config[:resource_group]} --server-name #{azure_config[:server_name]} --backup-name #{backup_name}

# Show backup details
az postgres flexible-server backup show --resource-group #{azure_config[:resource_group]} --server-name #{azure_config[:server_name]} --backup-name #{backup_name}
```
...
if backup_success
  ...
  puts "   az postgres flexible-server restore --resource-group #{azure_config[:resource_group]} --name NEW_SERVER_NAME --source-server #{azure_config[:server_name]} --backup-name #{backup_name}"
else
