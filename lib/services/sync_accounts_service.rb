...
# 2025-06-10 21:45:00 - Wrapped in Services module for Rails autoloading compatibility
...
module Services
    ...
    if user_data[:chatwoot_user_id].present?
      ...
      if user
        ...
        if account_user
          ...
          # User exists and is in account
          log_info("User is already active in account", { id: user.id, account_id: account.id })
  ...
  def reactivate_user(user, account_id)
    ...
    if account_user
      log_info("User already in account", { user_id: user.id, account_id: account_id })
    end
  end
  ...
  def assign_users_to_inboxes(account)
    ...
    log_info("Inbox assignment skipped - manual assignment required", { 
      account_id: account.id,
      inbox_count: inboxes.count,
      user_count: active_users.count,
      note: "Users have been added to account but inbox assignment must be done manually"
    })
    
    # Note: Inbox assignment varies by Chatwoot version
    # In newer versions, use: InboxMember.find_or_create_by(inbox: inbox, user: user)
    # For now, skipping automatic assignment
  end
